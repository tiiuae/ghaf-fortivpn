...
mod tests {
    ...
    #[test]
    fn accepts_valid_gateway_hosts() {
        for gateway in ["vpn.example.com", "192.0.2.1", "2001:db8::1"] {
            assert!(validate_hostname(gateway).is_ok(), "rejected {gateway}");
        }
    }

    #[test]
    fn rejects_gateway_urls_and_invalid_labels() {
        for gateway in [
            "",
            ...
            assert!(validate_hostname(gateway).is_err(), "accepted {gateway}");
        }
    }
    ...
    fn normalizes_sha256_fingerprint() {
        ...
        assert_eq!(
            normalize_fingerprint(fingerprint).unwrap(),
            Some("aabbccddeeff00112233445566778899aabbccddeeff00112233445566778899".into())
        );
        assert_eq!(normalize_fingerprint("").unwrap(), None);
    }

    #[test]
    fn rejects_malformed_fingerprints() {
        for fingerprint in ["abcd", &"g".repeat(64), &"a".repeat(63)] {
            assert!(normalize_fingerprint(fingerprint).is_err());
        }
    }

    #[test]
    fn rejects_conflicting_certificate_inputs() {
        let result = validate_request(b"bundle", "", b"certificate", b"key", "", b"");
    ...
    #[test]
    fn requires_certificate_and_private_key_together() {
        let result = validate_request(b"", "", b"certificate", b"", "", b"");
    ...
    #[test]
    fn rejects_password_without_corresponding_material() {
        assert_eq!(
            validate_request(b"", "password", b"", b"", "", b"")
                .unwrap_err()
                .0,
            "A PKCS#12 password was supplied without a bundle"
        );
        assert_eq!(
            validate_request(b"", "", b"", b"", "password", b"")
                .unwrap_err()
                .0,
            "A key password was supplied without a private key"
        );
    ...
    #[test]
    fn rejects_control_characters_in_profile_text() {
        assert!(validate_plain_text("valid name", 128, "empty", "invalid").is_ok());
        assert_eq!(
            validate_plain_text("invalid\nname", 128, "empty", "invalid")
                .unwrap_err()
                .0,
            "invalid"
        );
    ...
    #[test]
    fn requires_password_or_client_certificate() {
        let parameters = ProfileParameters {
            name: "Corporate VPN",
        ...
        assert_eq!(
            build_settings(&parameters, "test-uuid", &empty_stored_materials())
                .unwrap_err()
                .0,
            "Enter a VPN password or provide a client certificate"
        );
